...
│   ├── jupyter/                        # JupyterHub Helm values (GPU/CPU profiles)
│   ├── expose/                         # Public HTTPS exposure: Ingress, managed certs, GoogleOAuth / vLLM API key
│   └── ops/                            # Node handoff / holder re-arm / node-rotation runbook
├── docs/                               # Structured documentation set (guides + Graphviz diagrams + HTML exports); start at docs/README.md
├── lab/                                # SP-0 IaC foundation (Cluster Toolkit, Kueue, GPU matrix) + reservations guide
├── bugfixes/                           # DWS zone-pinning & 10-minute node-reclaim bug reports
...
graph TB
    subgraph Client ["Simplified User & Control Access"]
        API_Call["Developer Workstation / API Script"] -->|HTTPS REST Request| CP["GKE Control Plane (1.34.8-gke.1278000 | Channel: None)"]
    end
    ...
    subgraph ClusterMesh ["AI Hypercomputer High-Performance Network Fabric (us-east4-a/b/c)"]
        subgraph Node1 ["A3 High-GPU Compute Node 1 (a3-highgpu-8g | COS 121)"]
            subgraph Host1 ["Host Compute Sub-System"]
                CPU1["64x vCPUs & 384GiB RAM"] --- SHM1["64GiB Shared IPC RAM (/dev/shm)"]
            end
...
1. **Explicit Kernel Pinning (Container-Optimized OS 121)**:
   ...
   - To prevent newer GKE versions (`1.36` bundled with `COS 129`) from causing compatibility faults, our deployment explicitly decouples master and node versions: the control plane stays unenrolled from auto-upgrades (`--release-channel=None`), while the node pool initializes pinned at **GKE `1.33.13-gke.1101000` (`--no-enable-autoupgrade`)**, preserving the exact allowed `N-1` Kubernetes skew tolerance.
...
3. **Option 1 Endpoint Security & Direct REST Execution**:
   ...
   - To achieve seamless execution without whitelisting delays, we employ **Option 1 Execution Model**:
     - [scripts/03_submit_job_direct_gcloud.py](scripts/03_submit_job_direct_gcloud.py) uses secure OAuth Bearer tokens (`gcloud auth print-access-token`) directly against your GKE Master endpoint via secure HTTPS REST JSON calls (`/api/v1/...` and `/apis/batch/v1/...`).
     - [scripts/03_submit_verification_job.sh](scripts/03_submit_verification_job.sh) detects local security restrictions instantly and automatically falls back to the direct REST API engine without stopping.
